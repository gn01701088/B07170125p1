...
class MainActivity : AppCompatActivity() ,OnMapReadyCallback{
    ...
    override fun onMapReady(map: GoogleMap) {
        ...
        marker.draggable(true)
        map.addMarker(marker)
        marker.position(LatLng(24.801792208608433,120.97154538132303))
        marker.title("新竹車站")
        marker.draggable(true)
        map.addMarker(marker)
        ...
        polylineOpt.add(LatLng(24.801792208608433,120.97154538132303))
        polylineOpt.color(Color.BLUE)
        val polyline=map.addPolyline(polylineOpt)
        polyline.width=10f
        map.moveCamera(CameraUpdateFactory.newLatLngZoom(
            LatLng(24.84214581455704,120.98325099464469),13f))
    }
}